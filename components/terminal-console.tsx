...
export function TerminalConsole() {
  ...
  return (
    <div className="liquid-glass overflow-hidden font-mono text-[11px] transition-all duration-300">
      {/* Terminal Title Bar */}
      <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-border bg-secondary">
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-[#ff5f56] opacity-90 cursor-pointer hover:opacity-100 transition-opacity" />
          <span className="w-3 h-3 rounded-full bg-[#ffbd2e] opacity-90 cursor-pointer hover:opacity-100 transition-opacity" />
          <span className="w-3 h-3 rounded-full bg-[#27c93f] opacity-90 cursor-pointer hover:opacity-100 transition-opacity" />
        </div>
        <span className="text-[10px] font-semibold text-muted-foreground">guest@djordy: ~</span>
        <div className="w-12" />
      </div>
      ...
      <div 
        ref={bodyRef}
        className="p-4 h-56 overflow-y-auto space-y-2 text-foreground bg-black/5 dark:bg-black/25 backdrop-blur-md"
      >
        {history.map((item, i) => (
      ...
      <form
        ...
        className="flex items-center gap-2 px-4 py-2 border-t border-border bg-card"
      >
